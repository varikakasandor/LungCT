...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase = 'train',split_comber=None):
        ...
        self.filenames=[]
        for idx in idcs:
            clean_path=os.path.join(data_dir, '%s_clean.npy' % idx)
            if os.path.exists(clean_path):
                self.filenames.append(clean_path)
            else:
                print(f"{idx} clean didn't exist.")
        ...
        for idx in idcs:
            clean_path=os.path.join(data_dir, '%s_label.npy' % idx)
            if os.path.exists(clean_path):
                l=np.load(clean_path)
                self.sample_bboxes.append(l)
            else:
                print(f"{idx} label didn't exist.")
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            ...
            xx,yy,zz = np.meshgrid(np.linspace(-0.5,0.5,imgs.shape[1]//self.stride), #EARLIER: only single /, not //
